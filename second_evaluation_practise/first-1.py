def isspellable(word1, word2):
    ...
    # changed to this as it makes use of set and issubset() for sets
...
def solutions(location, puzzle, minimum=4):
    ...
    with open(location, encoding='utf8') as f:
        for w in f:
            w = w.strip()
            if len(w) >= minimum and isspellable(w, puzzle) and {c.lower() for c in puzzle if c.isupper()}.issubset(w):
                result.add(w)
    return result
